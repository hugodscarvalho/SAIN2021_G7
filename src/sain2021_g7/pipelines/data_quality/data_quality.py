...
def data_quality(prodsales: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_data_counts = pd.DataFrame(
        prodsales.isnull().sum(), columns=["Missing Values"]
    )
    ...
    data_quality_report = (
        data_types.join(row_count).join(missing_data_counts).join(unique_value_counts)
    )
